...
async function deploy() {
    ...
    const subscriptions = await web3.deploy('Subscriptions', [contractRegistry.address, registryAdmin, erc20.address, options.genesisRefTimeDelay || 3*60*60, options.minimumInitialVcPayment, [], 1000000, ZERO_ADDR]);
    ...
    const committee = await web3.deploy('Committee', [contractRegistry.address, registryAdmin, options.maxCommitteeSize]);
